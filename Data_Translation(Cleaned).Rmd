...
The above code shows model 1, which is a fixed effects model showing employment trends while controlling for variation within time, and model 2, a multivariate regression model showing the difference in employment trends before and after Covid, as well as in the retail industry. The results of the regression models are shown in the table above. 
...
Model 3: a multivariate regression model showing the difference in employment trends before and after Covid, as well as in the retail industry controlling for age. 

Model 4: a multivariate regression model showing the difference in employment trends before and after Covid, as well as in the retail industry controlling for sex. 

Model 5: a multivariate regression model showing the difference in employment trends before and after Covid, as well as in the retail industry controlling for age and sex. 
...
```{r}
#the bp test for model 1 is not working, it's probably cuz it only has an interaction term
#bptest(model1)
bptest(model2)
bptest(model3)
bptest(model4)
...
#Why did we plot "model6youngbinaryeffect" and not the code below?
